...
public class FriendsSearchListLoader extends AsyncTaskLoader<List<Friend>> {
    ...
    public void deliverResult(List<Friend> friends) {
        if (isReset()) {  //if there was a reset in this process (another process is in progress)
            if (friends != null)
                mCursor.close();  //we are letting another process continue and closing this cursor
        }
